...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Pre-fill the server IP address from the user query
    ...
    private void saveDataToPublicDocuments(List<DataObject> dataToSave) {
        ...
        // Use ContentResolver and ContentValues to save to the public Documents directory
        // This is the modern, Scoped Storage-compliant way for Android 10+
    ...
    private void uploadBulkData(List<DataObject> data, String serverIp) {
        ...
        // The patientId and experimentId are the same for all objects in a batch
        String patientId = data.get(0).getPatientId();
        String experimentId = data.get(0).getExperimentId();

        // Construct the URL for the new bulk upload endpoint
        String url = "http://" + serverIp + "/patient/" + patientId + "/data/bulk";

        // CORRECTION: Create a wrapper map to structure the JSON payload correctly.
        // The backend expects a JSON object like: {"experimentId": "...", "data": [...]}
        Map<String, Object> requestBodyMap = new HashMap<>();
        requestBodyMap.put("experimentId", experimentId);
        requestBodyMap.put("data", data); // 'data' is the List<DataObject>

        // Serialize the entire map into a single JSON object string.
        String jsonPayload = new Gson().toJson(requestBodyMap);
        ...
        StringRequest request = new StringRequest(Request.Method.POST, url,
            ...
            @Override
            public byte getBody() {
                // The jsonPayload now correctly represents the structured JSON object.
