...
- Core API Endpoint Test Procedures
...
4.	Go to the "Authorization" tab. 
        ...
        o	Username: {{username}} (e.g., user).
        o	Password: {{password}} (e.g., password).
5.	Send the request.
6.	Expected Response: 
...
**E. Testing Patient Management Endpoints**
...
Procedure:
   ...
   3.	Enter the Request URL: {{base_url}}/patient.
   4.	Go to the "Authorization" tab:
        o	Set Auth Type: Basic Auth. 
        o	Username: {{username}}.
        o	Password: {{password}}.
   ...
   6.	Enter the following sample JSON payload: 
   ...
   8.	Expected Response:
       ...
       o	Body: A JSON object containing the newly created patient's ID and other details. The exact structure may vary, but a common pattern is: 
...
•	Prerequisite: A valid patient_id obtained from a successful POST /patient request (see II.B.1). Use the {{patient_id}} environment variable if set.
   1.	Create a new request in Postman.
   2.	Set the method to GET.
   3.	Enter the Request URL: {{base_url}}/patient/{{patient_id}}. Replace {{patient_id}} with an actual, existing patient ID if not using environment variables.
   4.	Go to the "Authorization" tab:
       o	Set Auth Type: Basic Auth.
       o	Username: {{username}}.
   ...
   5.	Send the request.
   6.	Expected Response (Successful Retrieval):
       o	Status Code: 200 OK.
...
-
   7.	Expected Response (Patient Not Found):
...
•	Prerequisite: It is advisable to have created at least two distinct patients using POST /patient to properly observe the list structure and potentially test pagination features if implemented.
   1.	Create a new request in Postman.
   2.	Set the method to GET.
   3.	Enter the Request URL: {{base_url}}/patients.
   4.	Go to the "Authorization" tab:
        o	Set Auth Type: Basic Auth.
        o	Username: {{username}}.
        o	Password: {{password}}.
...
3.	Enter the Request URL: {{base_url}}/patient/{{patient_id}}.
4.	Go to the "Authorization" tab:
    o	Set Auth Type: Basic Auth.
    o	Username: {{username}}.
    o	Password: {{password}}.
...
6.	Enter a JSON payload with the fields to be updated. For example, to update the patient's age: 
...
**F. Testing Experiment Management Endpoints**
...
1.	Create a new request in Postman.
2.	Set the method to POST.
3.	Enter the Request URL: {{base_url}}/experiment.
4.	Go to the "Authorization" tab:
    o	Set Auth Type: Basic Auth.
    o	Username: {{username}}.
    o	Password: {{password}}.
...
1.	Create a new request in Postman.
2.	Set the method to GET.
3.	Enter the Request URL: {{base_url}}/experiment/{{experiment_id}}.
4.	Go to the "Authorization" tab (Basic Auth: {{username}}/{{password}}).
5.	Send the request.
6.	Expected Response (Successful):
    o	Status Code: 200 OK.
    o	Body:
...
-
    7.	Expected Response (Experiment Not Found):
        o	Status Code: 404 Not Found.
        o	Body: {"error": "Experiment not found"} (or similar).
    8.	Action: Verify retrieved data. Test with a valid and an invalid experiment_id.
...
Procedure:
   1.	Create a new request in Postman.
   2.	Set the method to GET.
   3.	Enter the Request URL: {{base_url}}/experiments.
   4.	Go to the "Authorization" tab (Basic Auth: {{username}}/{{password}}).
   5.	Send the request.
   6.	Expected Response:
       o	Status Code: 200 OK.
       o	Body: An array of experiment objects. An empty array `` if no experiments exist.
   7.	Action: Verify the response structure. Consider pagination if applicable.
...
•	Prerequisite: A valid experiment_id (e.g., {{experiment_id}}).
   ...
   3.	Enter the Request URL: {{base_url}}/experiment/{{experiment_id}}.
   4.	Go to the "Authorization" tab (Basic Auth: {{username}}/{{password}}).
   5.	Go to the "Body" tab (raw, JSON). Payload example: 
...
- 
    ...
    8.	Action: Verify with a subsequent GET /experiment/{{experiment_id}}. Test with non-existent ID for 404.
...
Procedure:
   ...
   1.	Create a new request in Postman.
   2.	Set the method to POST.
   3.	Enter the Request URL: {{base_url}}/patient/{{patient_id}}/data.
   4.	Go to the "Authorization" tab:
        o	Set Auth Type: Basic Auth.
        o	Username: {{username}}.
        o	Password: {{password}}.
   5.	Go to the "Body" tab.
        o	Select "raw".
        o	Choose "JSON" from the dropdown.
   6.	Enter a sample JSON payload, including the experimentId: 
...
Procedure:
   ...
   1.	Create a new request in Postman.
   2.	Set the method to GET.
   3.	Enter the Request URL: {{base_url}}/patient/{{patient_id}}/data?experimentId={{experiment_id}}.
   4.	Go to the "Authorization" tab (Basic Auth: {{username}}/{{password}}).
   5.	Send the request.
   6.	Expected Response:
       o	Status Code: 200 OK.
...
Procedure:
   ...
   1.	Create a new request in Postman.
   2.	Set the method to GET.
   3.	Enter the Request URL: {{base_url}}/patient/{{patient_id}}/data/{{data_point_id}}.
   4.	Go to the "Authorization" tab (Basic Auth: {{username}}/{{password}}).
   5.	Send the request.
   6.	Expected Response (Successful):
        o	Status Code: 200 OK.
...
-
    7.	Expected Response (Data Point Not Found): 
        o	Status Code: 404 Not Found.
        o	Body: {"error": "Data point not found"} (or similar).
    8.	Action: Verify that the retrieved data point matches the data that was originally posted. Test with an invalid or non-existent data_point_id to confirm 404 Not Found handling. The existence of a dataPointId upon creation implies its utility for later retrieval or management of that specific entry.
...
Procedure:
   ...
   3.	Enter the Request URL: {{base_url}}/patient/{{patient_id}}/data/{{data_point_id}}.
   4.	Go to the "Authorization" tab (Basic Auth: {{username}}/{{password}}).
   5.	Go to the "Body" tab (raw, JSON). Enter a payload with the fields to be updated. For example, correcting a heart rate value: 
...
This will display a graph of memory usage (in MiB) versus time (in seconds).
    o	Look for trends: Is memory usage stable, or does it continuously increase (suggesting a leak)?
    o	mprof plot -s: This option plots trend lines and their numeric slopes. A slope significantly greater than zero over a substantial period can be a strong indicator of a memory leak.
